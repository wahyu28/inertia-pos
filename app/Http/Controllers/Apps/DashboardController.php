...
class DashboardController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        //sum profits today
        $sum_profits_totay = Transaction::whereDay('created_at', $day)->sum('total');

        //get product limit stock
        $product_limit_stock = Transaction::with('category')->where('stock', '<=', 10)->get();
        ...
        return Inertia::render('Apps/Dashboard/Index', [
            ...
            'sum_profits_today' => (int) $sum_profits_totay,
            'products_limit_today' => $product_limit_stock,
            'product' => $product,
            'total' => $total
        ]);
    }
